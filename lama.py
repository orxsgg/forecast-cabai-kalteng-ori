...
df = df[
    df["Province_Name"] == "Kalimantan Tengah"
]
...
def home():
    ...
    return {
        "message":
        "API Forecasting Harga Cabai Aktif"
    }
...
def predict(data: ForecastRequest):
    ...
    hari_prediksi = data.hari_prediksi
    ...
    current_seq = last_sequence.reshape(
        1,
        30,
        1
    )
    ...
    hasil_rupiah = scaler.inverse_transform(
        hasil_prediksi
    )
    ...
    prediksi_besok = round(
        float(hasil_rupiah[0][0]),
        2
    )
